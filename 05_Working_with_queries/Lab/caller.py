...
# ------------- 2. Find Author Nationalities -------------
def find_authors_nationalities() -> str:
    # all_authors = Author.objects.filter(nationality__isnull=False)
    # authors_info = []
    #
    # for author in all_authors:
    #     authors_info.append(f"{author.first_name} {author.last_name} is {author.nationality}")
...
def order_books_by_year() -> str:
    ...
    #     books_info.append(f"{book.publication_year} year: {book.title} by {book.author}")
    #
    # return "\n".join(books_info)
...
def delete_review_by_id(id: int) -> str:
    reviewer_obj = Review.objects.get(pk=id)

    message = f"Review by {reviewer_obj.reviewer_name} was deleted"
...
def filter_authors_by_nationalities(nationality: str) -> str:
    ...
    info = "\n".join([f"{author.first_name} {author.last_name}" if not author.biography else f"{author.biography}"
...
def filter_authors_by_birth_year(year_1: int, year_2: int) -> str:
    filtered_authors = (Author.objects.filter(birth_date__year__gte=str(year_1), birth_date__year__lte=str(year_2))
                        .order_by('-birth_date'))

    return "\n".join(f"{author.birth_date}: {author.first_name} {author.last_name}" for author in filtered_authors)
...
def change_reviewer_name(name_1: str, name_2: str) -> QuerySet:
    Review.objects.filter(reviewer_name=name_1).update(reviewer_name=name_2)
