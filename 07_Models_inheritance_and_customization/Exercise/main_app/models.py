...
class Message(models.Model):
    ...
    def reply_to_message(self, reply_content: str) -> 'Message':
        message = Message.objects.create(sender=self.receiver, receiver=self.sender, content=reply_content)

        return message

    def forward_message(self, receiver: UserProfile) -> 'Message':
        message = Message.objects.create(sender=self.receiver, receiver=receiver, content=self.content)
...
class Room(models.Model):
    ...
    def clean(self) -> None:    # used for model validation
...
class BaseReservation(models.Model):
    ...
    # usually a defined as a property
    def reservation_period(self) -> int:
        duration_of_stay = (self.end_date - self.start_date).days
    ...
    def is_available(self) -> bool:
        reservations = self.__class__.objects.filter(       # това филтрира класа, който наследява BaseReservation
            room=self.room,
